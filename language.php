...
switch (APP) {
  case 'lidarr';
    ...
    define('EXISTING_LANGUAGE_ROOT', LIDARR_LANGUAGE_ROOT);
  ...
  case 'radarr';
    ...
    define('EXISTING_LANGUAGE_ROOT', RADARR_LANGUAGE_ROOT);
  ...
  case 'readarr';
    ...
    define('EXISTING_LANGUAGE_ROOT', READARR_LANGUAGE_ROOT);
  ...
  case 'sonarr';
    ...
    define('EXISTING_LANGUAGE_ROOT', SONARR_LANGUAGE_ROOT);
    break;
  default:
    define('TRANSLATE_ROOT', '');
    define('EXISTING_LANGUAGE_ROOT', '');
    break;
}
...
$searchFor = array(
  ...
  array(
    'apply' => ((in_array('regex_defineApos', $regexs)) ? true : false),
    'name' => 'defineApos',
    'regex' => '/(const|let )(.*) = \'([^"]+)\'/i'
  ),
  array(
    'apply' => ((in_array('regex_mlHelpTexts', $regexs)) ? true : false),
    'name' => 'mlHelpTexts',
    'regex' => 'helpTexts='
  ),
);

$searchFiles = array('js', 'trx');
...
if (file_exists(EXISTING_LANGUAGE_ROOT)) {
  $existingLanguageArray = json_decode(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', file_get_contents(EXISTING_LANGUAGE_ROOT)), true);
}
          ...
          if ($matches) {
            foreach ($matches as $matchIndex => $matchData) {
              ...
              if ($regexOptions['name'] == 'defineQuote' || $regexOptions['name'] == 'defineApos') {
                ...
                if (
                  strpos($matchData[0], 'translate(') === false && $matchData[3] != 'MMM D YYYY' && $matchData[3] != 'yyyy-mm-dd' && trim($matchData[2]) != 'protocol' && trim($matchData[2]) != 'key' &&
              ...
              elseif ($regexOptions['name'] == 'attrQuotes' || $regexOptions['name'] == 'attrInterp') {
                ...
                if (
                  strpos($matchData[2], 'translate(') === false && strpos($matchData[2], 'formatDateTime(') === false && strpos($matchData[2], 'titleCase(') === false &&
...
if ($mergedLanguageKeys) {
  ...
  if (!DRY_RUN) {
    file_put_contents(EXISTING_LANGUAGE_ROOT, $newJson);
  }
}
