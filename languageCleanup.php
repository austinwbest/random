...
define('LIDARR_LANGUAGE_ROOT', 'C:\\Repositories\\Lidarr-Fork\\src\\NzbDrone.Core\\Localization\\Core\\');
...
$codebase = array('front' => array('files'      => recursive_read(FRONTEND_ROOT), 
                  ...
                  'back'  => array('files'      => recursive_read(BACKEND_ROOT, false, true), 
...
switch (TYPE)
{
  case 'unused':
    ...
    $results = array('frontendScanned'  => $codebase['front']['fileCount'],
    ...
    foreach ($codebase as $where => $details)
    {
      foreach ($details['files'] as $filePath)
      {
        ...
        if (in_array($extension, $details['ext']))
        {
          ...
          preg_match($details['regex_m'], $file, $matches);

          if ($matches)
          {
            if (!$lang[$matches[1]])
            {
              $codebaseKeys[$matches[1]][] = $filePath;
            }
          }
        }
      }
